...
export async function run({ page, human, logger, input, cfg, dryRun }) {
  ...
  if (stillOpen) {
    throw new Error('⛔ מסך התוצאה עדיין פתוח אחרי הקליק על אישור השני — סימן שהקליטה לא הושלמה. נמדד 08/09/2026: הרצה עם המצב הזה שדיווחה "ok" הקימה 0 מתוך 10 פריטים בפועל. לא לדווח הצלחה — לבדוק ידנית בקומקס ולהריץ items-export להשוואה לפני שמנסים שוב.');
  }
