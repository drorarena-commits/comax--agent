...
export const meta = {
  ...
  writes: true,
  input: {
    file: 'נתיב לקובץ ההקמה (xlsx), יחסי ל-root או מוחלט. חובה',
    probe: 'boolean — true למפות את הדיאלוג ולעצור בלי לגעת בכלום',
    ...
    allowUpdate: 'boolean — true לאפשר גם עדכון פריטים קיימים ("הקמת פריט" ריק). ברירת המחדל: הקמה בלבד',
    priceDate: 'תאריך "מחיר מכירה נכון לתאריך" בפורמט dd/mm/yyyy. ברירת המחדל: היום',
    previewOnly: 'boolean — true לוחץ רק על הכפתור הירוק הראשון (בניית התצוגה המקדימה), ועוצר. לא נוגע בכפתור השני, הקליטה. דורש --confirm כדי בכלל להגיע למסך הזה',
    partial: 'boolean — true לאשר קובץ עם תת-קבוצה של העמודות, לעדכון שדה בודד (מחיר, למשל) בפריטים קיימים. חייב מק"ט וברקוד, ולפחות שדה נתונים אחד. לא מקים פריטים',
  },
  // `partial` הוא עדכון פריטים קיימים ולכן אינו יכול לרוץ בלי `allowUpdate`.
  // אף אחד מהשניים אינו חובה בפני עצמו, וזה בדיוק מה שהמילה "חובה" אינה יודעת
  // לבטא — ולכן זה precheck. המשימה כבר זורקת על זה מבפנים, אחרי לוגין ומושב,
  // וזו הכותבת הבלתי הפיכה: הלוגין שנחסך כאן הוא לוגין שהיה נגמר בכלום.
  precheck: (input) => {
    if (input.partial && !input.allowUpdate) {
      return '--partial הוא עדכון פריטים קיימים ולכן דורש גם --allowUpdate.';
    }
    return null;
  },
};
