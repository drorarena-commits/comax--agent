...
export function renderQuestions(questions, codes) {
  ...
  for (const q of questions) {
    ...
    if (groupKey !== lastSku) {
      ...
      // ⛔ אב שנשאל עליו הוא אב **חדש**, כלומר אין לו שורה בכרטיס ואין לו שדה
      // חלופי שלם בשום מקור בתוך הגבול — ולכן "חסר". לא "המק"ט בלי AR": הזהות
      // הזאת מתקיימת רק ב-599 מתוך 728 אבות, ונגזרת שנראית נכונה גרועה מ"חסר".
      for (const sku of q.skus) {
        lines.push('    אב ' + sku + ' · חלופי חסר · ' + (q.desc || 'חסר'));
      }
      lastSku = groupKey;
    }
