 ...
 */
/**
 * **שלוש עמודות, בסדר הזה** — נמדד במסך `יבוא מחירונים מקובץ CSV`
 * (`Erp/Mhr/Imp_MhrTogether/Imp_MhrTogether_Exl.asp`, נפתח מ-`a6` → "יבוא
 * מאקסל", 07/09/2026). תחת "שדות ליבוא" מסומנות בברירת מחדל בדיוק שלוש
 * תיבות: `SwMhrKod` מחירון · `SwBarCode` ברקוד · `SwMhr` מחיר.
 *
 * ⚠️ **העמודה הראשונה היא מספר המחירון, לא קוד הספק** — דרור העיר על זה
 * במפורש, וצדק. ראה `PRICE_LISTS`.
 * ⚠️ **המסך דורש CSV, וכותרת לכל עמודה** — כתוב על המסך עצמו במילים האלה,
 * וזה גם מה שדף העזרה שדרור מסר מלמד להכין.
 */
export const COST_HEADERS = ['מחירון', 'ברקוד', 'מחיר'];

/**
 * מספרי המחירונים, כפי שנקראו מהרשת ב-`a6` (07/09/2026).
 *
 * ⚠️ **מספר המחירון אינו קוד הספק, למרות שהם דומים עד כדי בלבול.** אצל שלושה
 * ספקים הם זהים (MGS `121001` · אוני ספורט `121011` · טלבר `121013`), ודווקא
 * אצל ספורט אנד מור **הם שונים**: קוד הספק `121012`, ומספר המחירון
 * **`1210012`** — אפס נוסף באמצע. זה יוצא דופן מול הדפוס של כל השאר, וייתכן
 * שזו שגיאת הקלדה בהגדרת המחירון. **הערך נלקח כפי שהוא מהמסך** ולא "מתוקן"
 * לדפוס, אבל צריך לאשר אותו לפני יבוא של 719 שורות לתוכו.
 */
export const PRICE_LISTS = {
  sale: 1,          // מכירה ראשי — 12,971 פריטים, כולל מע"מ
  club: 2,          // מכירה מועדון — ריק
  lastBuy: 9,       // קניה אחרונה — 11,248 פריטים, אופי "קניה"
  groups: 111,      // מחירון קבוצות — 81 פריטים
  mgs: 121001,      // MGS — 10,112 פריטים, אופי "קניה"
  sportAndMore: 1210012, // ספורט אנד מור — **ריק**, אופי "קניה"
  // נקראו מהרשת ב-14/09/2026, בהרצת יבוא למחירון 1. לא היו במיפוי המקורי.
  oniSport: 121011, // אוני ספורט — 17 פריטים
  talbar: 121013,   // טלבר קניה — פריט אחד
  ayg: 121014,      // א.י.ג. יבוא ויצוא — ריק
};

/** איזה ערך נכנס לכל הרצה, ולאיזה מחירון. המפתח הוא שם השדה ב-`summarize()`. */
export const RUNS = {
  supplier: {
    field: 'weighted',
    list: PRICE_LISTS.sportAndMore,
    label: 'מחירון ספק — ספורט אנד מור',
    file: 'מחירון-ספק-ספורט-אנד-מור',
  },
  last: {
    field: 'last',
    list: PRICE_LISTS.lastBuy,
    label: 'מחיר קניה אחרונה',
    file: 'מחיר-קניה-אחרונה',
  },
};
