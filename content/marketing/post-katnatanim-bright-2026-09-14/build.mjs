...
const INK = '#1C1C1B';
const BG = '#FFFDF7';
const SIZE = 1080; // ריבוע — פוסט רגיל, לא נחתך ברשת הפיד כמו פורטרט 4:5
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .clip{position:absolute;left:0;top:0;width:${SIZE}px;height:${SIZE}px;overflow:hidden}
  .blob{position:absolute;border-radius:50%;opacity:.55;filter:blur(2px)}
  .logo-top{position:absolute;right:60px;top:50px}
  ...
  footer{position:absolute;left:60px;right:60px;top:1010px;height:32px;
         display:flex;align-items:center;justify-content:flex-start;direction:ltr}
  .card{position:absolute;left:140px;top:70px;width:800px;height:460px;border-radius:32px;
        box-shadow:0 24px 48px rgba(28,28,27,.12)}
  .photo{position:absolute;left:31px;top:30px;width:400px;height:400px;overflow:hidden;
  ...
  .badge{position:absolute;left:620px;top:110px;width:270px;color:#fff;text-align:center;
        font-weight:900;font-size:28px;padding:16px 10px;border-radius:20px;line-height:1.3;
        box-shadow:0 8px 20px rgba(0,0,0,.18)}
  .pname{position:absolute;right:100px;width:880px;top:560px;text-align:right;color:${INK};
        font-weight:700;font-size:44px;line-height:1.25}
  .price{position:absolute;right:100px;top:700px;color:${BLUE};font-weight:900;font-size:72px}
  .pdesc{position:absolute;right:100px;width:880px;top:800px;text-align:right;color:${INK};
        opacity:.65;font-weight:400;font-size:28px;line-height:1.5}
  .detail{position:absolute;right:100px;width:880px;text-align:right}
  .detail .row{display:flex;flex-direction:row-reverse;align-items:baseline;gap:20px;margin-bottom:38px}
  .detail .k{color:${BLUE};font-weight:700;font-size:32px;min-width:200px}
  .detail .v{color:${INK};font-weight:400;font-size:32px}
</style></head><body><div class="clip">${inner}
  <footer>${logoSvg(INK, 70)}</footer>
</div></body></html>`;
...
const cover = shell(`
  ...
  <div class="logo-top">${logoSvg(BLUE, 160)}</div>
  <div class="title" style="font-size:88px;top:420px">קטנטנים. 🧸</div>
  <div class="sub" style="font-size:50px;top:590px">קטגוריה חדשה אצלנו</div>
  <div class="desc" style="font-size:27px;top:660px">ציוד שחייה צבעוני לגיל הרך — עכשיו באתר ובחנות בוינגייט</div>
  <div style="position:absolute;right:100px;width:880px;top:940px;text-align:left;color:${INK};opacity:.6;font-weight:400;font-size:24px">החליקו ←</div>
`);
...
const other = (img, name, price, badge, badgeColor, cardColor) => shell(`
  ...
  <div class="price" style="font-size:52px">${price}</div>
...
const capSlide = other('cap-roy.jpg', capName, 'מ-69.90 ₪', 'לגילאי 6-36 חודשים', '#00B4A6', '#FFF3D6');
const warmsuitSlide = other('warmsuit.jpg', 'בגד ים שומר חום — Neoprene Warmsuit', '159.90 ₪', 'לגילאי 1-6', '#FFA800', '#E4F6FF');
const gogglesSlide = other('goggles.jpg', gogglesName, '109.00 ₪', 'לגילאי 2-5', BLUE, '#FFE8F1');

const details = shell(`
  <div class="blob" style="right:-100px;top:-100px;width:340px;height:340px;background:#FFD93D"></div>
  <div class="title" style="font-size:64px;top:160px">כל הפרטים</div>
  <div class="detail" style="top:380px">
...
const closing = shell(`
  ...
  <div class="title" style="font-size:70px;top:420px;text-align:center;right:100px;width:880px">בואו לבדוק —
  באתר או בחנות בוינגייט</div>
  <div style="position:absolute;right:100px;width:880px;top:580px;text-align:center;color:${BLUE};
       font-weight:700;font-size:40px">arenaisrael.co.il</div>
`);
