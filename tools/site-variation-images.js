 ...
 *
 * מקור השכפול היחיד המותר הוא **ווריאציה אחות באותו `pa_color`** שיש לה תמונה
 * משלה. לא גלריית ההורה (אי אפשר לדעת איזו תמונה שייכת לאיזה צבע), לא
 * `ux_image` של המונח (זה ריבוע צבע 50×50 ומשותף לכל המוצרים), ולא
 * `commercekit_*` — CommerceKit הוסר מהאתר, והערכים שנשארו שם הם שאריות של
 * שכפולי מוצרים ומצביעים על צבעים שאינם של המוצר כלל.
 *
